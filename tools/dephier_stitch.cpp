...
// a tile-local terminal or a seam exit. Phase 2 (CHAIN): follow those exits across tiles until a
// terminal, reading only the O(boundary) Phase-1 results plus edge labels -- never another tile's
// interior. Order-independent; the resolved global labels are written into gLabel together at the end.
static void resolve_conduits(StitchState &st){
  const auto& full=st.full; const auto& bounds=st.bounds; const int ntiles=st.ntiles;
  auto& tiles=st.tiles; auto& gLabel=st.gLabel;
  const auto tile_of=[&](int x){ return st.tile_of(x); };
  const auto drain=[&](int x,int y,int &lx,int &ly,bool &to_ocean){ return st.drain(x,y,lx,ly,to_ocean); };
...
static void print_usage(const char *prog){
  std::cout <<
    ...
    "\n"
    "In-process tiled DepressionHierarchy 'stitch': builds the hierarchy from column\n"
    "tiles and diffs it against a full-grid serial build (validation harness).\n"
    "\n"
    ...
    "                   (e.g. \"2,4,6\" -> 4 tiles).\n"
    "  [Flat Halo Cap]  Optional. Max halo columns per side for the adaptive-halo flat\n"
    "                   resolution. Default: unbounded = bit-identical to serial. A\n"
    "                   finite cap bounds the footprint but may not be serial-identical\n"
    "                   inside flats wider than the cap.\n"
    "\n"
    "Modes (environment variables; set to any value to enable):\n"
    "  DH_FLAT_PARTITION_REPLAY  Reproduce serial's EXACT flat-label partition so the\n"
    "                            tree is bit-identical to serial. Default off.\n"
